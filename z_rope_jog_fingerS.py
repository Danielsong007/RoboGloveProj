...
def main():
    try:
        ...
        Samples_Hand_S = deque(maxlen=3)  # 自动丢弃旧数据
        Samples_Hand_L = deque(maxlen=6)  # 自动丢弃旧数据
        Flag_Weight=0
        Sig_Grasped=150
        ...
        while True:
            ...
            Samples_Hand_S.append(Hand_S)
            Ave_Hand_S=np.mean(Samples_Hand_S)
            Samples_Hand_L.append(Hand_S)
            Ave_Hand_L=np.mean(Samples_Hand_L)

            if Ave_Hand_S>Sig_Grasped and Flag_Weight==0:
                print('Measure Mode')
                mode=1
                ...
                if Ave_Hand_L>Sig_Grasped and Flag_Weight==1:
                    mode=2 # Loop Mode
                    Rope_B=Weight
                ...
                myXYZ.AxisMode_Jog(3,30,Vgoal)
                Vgoal_L=Vgoal
                print('Mode:',mode,'Hand_S',Hand_S,'Rope_S:', Rope_S,'Vgoal',int(Vgoal), 'Diff:',int(diff))
