...
class RuiFenAngleSensor:
    ...
    @staticmethod
    def parse_angle_data(data_frame, measurement_range): # 解析角度数据
    ...
    def read_angles(self, measurement_range=10): # 读取传感器角度数据
        ...
        x_angle, y_angle = self.parse_angle_data(response, measurement_range)
        return x_angle, y_angle
...
if __name__ == "__main__":
    sensor = RuiFenAngleSensor()
    try:
        # sensor.set_mode()
        while True:
            x_angle, y_angle = sensor.read_angles(measurement_range=10)
