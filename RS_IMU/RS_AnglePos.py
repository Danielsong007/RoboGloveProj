...
velocity = np.array([0.0, 0.0, 0.0])  # 初始速度 (vx, vy, vz)
...
try:
    while True:
        ...
        if gyro_frame:
            ...
            # 去除重力分量，得到线性加速度
            gravity = np.array([0.0, 0.0, 9.81])  # 重力加速度 (m/s²)
            rotation_matrix = quaternion_to_rotation_matrix(q)
            gravity_world = np.dot(rotation_matrix, gravity)
            linear_acceleration = np.array([ax, ay, az]) - gravity_world
